...
function roster(msg, record, tag) {
	return Promise.resolve().then(() => {
		if (tag) {
			...
			return wotblitz.clans.list(tag, null, null, ['clan_id', 'tag']).then(list => {
			...
			return wotblitz.clans.accountinfo(record.account_id, null, 'clan_id').then(accountinfo => {
		...
		return wotblitz.clans.info(clan_id, 'members', ['clan_id', 'name', 'members.account_name', 'members.role'])
			.then(info => info[clan_id]);
	}).then(clanInfo => {
