...
function roster(msg, record, tag) {
	var promiseClanId = new Promise((resolve, reject) => {
      ...
      // if a tag does not follow the rules
			if (!tag.match(/^[A-Z0-9-_]{2,5}$/)) return reject('invalid tag');
			...
			wotblitz.clans.accountinfo([record.account_id], [], ['clan_id'], null, (err, accountinfo) => {
				if (err) return reject(err);
				if (!accountinfo[record.account_id]) return reject('no clan');
	...
	return style ? style + escapedName + style : escapedName;
      ...
      // rejection via user input is not actually an error, do not report as an error
			if (reason === 'not found' || reason === 'invalid tag' || reason === 'no clan') return resolve(null);

			reject(reason);
		});   // end "promiseClanId.then" resolve
