...
function masteryList(msg, record, level) {
	...
	return wotblitz.tanks.stats(record.account_id, null, null, null, ['mark_of_mastery', 'tank_id']).then(stats => {
		...
		var limit = 100;
		var reqVehicles = [];
		var fields = ['name', 'teir', 'nation'];
		var i;

		for (i = 0; i < tankIds.length; i += limit) {
			reqVehicles.push(wotblitz.encyclopedia.vehicles(tankIds.slice(i, i + limit), null, fields));
		}
