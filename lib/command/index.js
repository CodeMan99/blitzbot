...
Commands.get = function getCommand(cmdName) {
	for (const key in Commands.prototype) {
		if (key === cmdName || Commands.prototype[key].options.alias === cmdName) {
			return key;
		}
	}
 ...
 * @param {string} [options.alias]             alternative name used for lookup by {Commands#get}
...
function createHelp() {
	...
	async function helpFn(msg, helpFor) {
		const lines = [];
		const name = this.client.user.username;
		const push = opt => {
			for (const sig of opt.signatures) {
		...
		const messages = await Promise.all(lines.map(send => msg.author.send(send)));
