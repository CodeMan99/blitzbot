...
function createHelp() {
	...
	function helpFn(msg, helpFor) {
		...
		if (helpFor) {
			...
			Object.keys(Commands.prototype).forEach(key => push(Commands.prototype[key].options));

			if (!Commands.has('help')) push(options);
		}

		return Promise.all(lines.map(send => msg.author.sendMessage(send))).then(messages => {
