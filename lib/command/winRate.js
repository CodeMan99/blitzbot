...
function twrFn(msg, authorRecord, tankName) {
	...
	return this.wotblitz.encyclopedia.vehicles(null, null, ['name', 'nation', 'tier']).then(vehicles => {
		...
		const exec = user ? this.db.findOne({_id: user.id}) : Promise.resolve(authorRecord);

		return exec.then(record => {
			if (!record && user) {
				return msg.reply(`I do not know who ${user.toString()} is. Sorry about that.`).then(sent => {
					return {sentMsg: sent};
				});
			}
