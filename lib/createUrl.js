...
function createApplicationUrl(permissions) {
	...
	if (pArr) {
		query.set('permissions', pArr.reduce((a, b) => a | b));
	} else if (!isNaN(permissions)) {
		query.set('permissions', permissions);
	} else {
		// default
		query.set('permissions', 0x0000400 | 0x0000800 | 0x0004000 | 0x0008000 | 0x0010000 | 0x0100000 | 0x0200000 | 0x2000000);
	}

	return discordAuthorizeURL + query.toString();
}
